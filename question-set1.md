# AWS Certified Cloud Practitioner Questions

## Question 1
What is the primary benefit of using AWS Lambda?

<details>
<summary>Click for options</summary>

1. A) Reduce costs for server-based applications
2. B) Automatically scale resources
3. C) Run code without provisioning servers
4. D) Improve data security

</details>

<details>
<summary>Click for the answer</summary>
C) Run code without provisioning servers
</details>

---

## Question 2
Which AWS service provides a scalable object storage solution?

<details>
<summary>Click for options</summary>

1. A) Amazon S3
2. B) Amazon EBS
3. C) Amazon RDS
4. D) Amazon EC2

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon S3
</details>

---

## Question 3
What does AWS stand for?

<details>
<summary>Click for options</summary>

1. A) Advanced Web Services
2. B) Amazon Web Services
3. C) Automated Web Solutions
4. D) Application Web Services

</details>

<details>
<summary>Click for the answer</summary>
B) Amazon Web Services
</details>

---

## Question 4
Which service is used for database management on AWS?

<details>
<summary>Click for options</summary>

1. A) Amazon DynamoDB
2. B) Amazon EC2
3. C) AWS Lambda
4. D) Amazon S3

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon DynamoDB
</details>

---

## Question 5
What is the primary use of Amazon EC2?

<details>
<summary>Click for options</summary>

1. A) Data warehousing
2. B) Hosting virtual servers
3. C) Content delivery
4. D) Load balancing

</details>

<details>
<summary>Click for the answer</summary>
B) Hosting virtual servers
</details>

---

## Question 6
What type of cloud computing service is AWS Lambda?

<details>
<summary>Click for options</summary>

1. A) Infrastructure as a Service (IaaS)
2. B) Platform as a Service (PaaS)
3. C) Software as a Service (SaaS)
4. D) Function as a Service (FaaS)

</details>

<details>
<summary>Click for the answer</summary>
D) Function as a Service (FaaS)
</details>

---

## Question 7
Which AWS service can be used to manage user access?

<details>
<summary>Click for options</summary>

1. A) Amazon IAM
2. B) Amazon VPC
3. C) Amazon S3
4. D) Amazon RDS

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon IAM
</details>

---

## Question 8
What is the purpose of Amazon CloudFront?

<details>
<summary>Click for options</summary>

1. A) Monitoring services
2. B) Content delivery network
3. C) Database management
4. D) Serverless computing

</details>

<details>
<summary>Click for the answer</summary>
B) Content delivery network
</details>

---

## Question 9
Which service allows you to run code in response to events?

<details>
<summary>Click for options</summary>

1. A) Amazon EC2
2. B) AWS Lambda
3. C) Amazon RDS
4. D) Amazon S3

</details>

<details>
<summary>Click for the answer</summary>
B) AWS Lambda
</details>

---

## Question 10
What is Amazon RDS primarily used for?

<details>
<summary>Click for options</summary>

1. A) Object storage
2. B) Relational database management
3. C) Big data processing
4. D) Load balancing

</details>

<details>
<summary>Click for the answer</summary>
B) Relational database management
</details>

---

## Question 11
Which AWS service is best for hosting a static website?

<details>
<summary>Click for options</summary>

1. A) Amazon EC2
2. B) Amazon S3
3. C) AWS Lambda
4. D) Amazon RDS

</details>

<details>
<summary>Click for the answer</summary>
B) Amazon S3
</details>

---

## Question 12
What is the purpose of Amazon VPC?

<details>
<summary>Click for options</summary>

1. A) Virtual private cloud
2. B) Data analytics
3. C) Load balancing
4. D) Object storage

</details>

<details>
<summary>Click for the answer</summary>
A) Virtual private cloud
</details>

---

## Question 13
Which service can be used to monitor AWS resources?

<details>
<summary>Click for options</summary>

1. A) AWS CloudTrail
2. B) Amazon CloudWatch
3. C) AWS Config
4. D) AWS Lambda

</details>

<details>
<summary>Click for the answer</summary>
B) Amazon CloudWatch
</details>

---

## Question 14
What is Amazon Elastic Beanstalk used for?

<details>
<summary>Click for options</summary>

1. A) Hosting applications
2. B) Storage management
3. C) Network configuration
4. D) Data processing

</details>

<details>
<summary>Click for the answer</summary>
A) Hosting applications
</details>

---

## Question 15
Which AWS service is used for serverless application development?

<details>
<summary>Click for options</summary>

1. A) AWS Lambda
2. B) Amazon EC2
3. C) Amazon RDS
4. D) AWS CloudFormation

</details>

<details>
<summary>Click for the answer</summary>
A) AWS Lambda
</details>

---

## Question 16
What does AWS CloudFormation do?

<details>
<summary>Click for options</summary>

1. A) Manages user permissions
2. B) Automates infrastructure deployment
3. C) Monitors application performance
4. D) Provides content delivery

</details>

<details>
<summary>Click for the answer</summary>
B) Automates infrastructure deployment
</details>

---

## Question 17
Which AWS service can be used to store files and access them via the internet?

<details>
<summary>Click for options</summary>

1. A) Amazon EBS
2. B) Amazon S3
3. C) Amazon RDS
4. D) AWS Lambda

</details>

<details>
<summary>Click for the answer</summary>
B) Amazon S3
</details>

---

## Question 18
What is the function of AWS Direct Connect?

<details>
<summary>Click for options</summary>

1. A) Secure data transfer
2. B) Dedicated network connection
3. C) Load balancing
4. D) Monitoring network traffic

</details>

<details>
<summary>Click for the answer</summary>
B) Dedicated network connection
</details>

---

## Question 19
Which AWS service is used for managing and deploying applications in containers?

<details>
<summary>Click for options</summary>

1. A) Amazon ECS
2. B) Amazon S3
3. C) Amazon RDS
4. D) AWS CloudFormation

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon ECS
</details>

---

## Question 20
What is the primary benefit of using Amazon Aurora?

<details>
<summary>Click for options</summary>

1. A) High availability
2. B) Low latency
3. C) Serverless computing
4. D) Object storage

</details>

<details>
<summary>Click for the answer</summary>
A) High availability
</details>

---

## Question 21
Which service allows you to create and manage virtual networks in AWS?

<details>
<summary>Click for options</summary>

1. A) Amazon VPC
2. B) AWS Lambda
3. C) Amazon RDS
4. D) Amazon S3

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon VPC
</details>

---

## Question 22
What is Amazon SNS primarily used for?

<details>
<summary>Click for options</summary>

1. A) Data storage
2. B) Message notification
3. C) Virtual machine management
4. D) Content delivery

</details>

<details>
<summary>Click for the answer</summary>
B) Message notification
</details>

---

## Question 23
Which AWS service is used for data analytics?

<details>
<summary>Click for options</summary>

1. A) Amazon Redshift
2. B) Amazon EC2
3. C) Amazon RDS
4. D) AWS Lambda

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon Redshift
</details>

---

## Question 24
What does AWS Auto Scaling do?

<details>
<summary>Click for options</summary>

1. A) Manages user permissions
2. B) Adjusts resources based on demand
3. C) Provides data analytics
4. D) Monitors application performance

</details>

<details>
<summary>Click for the answer</summary>
B) Adjusts resources based on demand
</details>

---

## Question 25
Which service allows you to host a relational database in AWS?

<details>
<summary>Click for options</summary>

1. A) Amazon RDS
2. B) Amazon S3
3. C) AWS Lambda
4. D) Amazon EC2

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon RDS
</details>

---

## Question 26
What is the purpose of AWS CodePipeline?

<details>
<summary>Click for options</summary>

1. A) Code version control
2. B) Continuous integration and delivery
3. C) Database management
4. D) Load balancing

</details>

<details>
<summary>Click for the answer</summary>
B) Continuous integration and delivery
</details>

---

## Question 27
Which AWS service provides a scalable email sending service?

<details>
<summary>Click for options</summary>

1. A) Amazon SES
2. B) Amazon SNS
3. C) AWS Lambda
4. D) Amazon RDS

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon SES
</details>

---

## Question 28
What does Amazon CloudTrail do?

<details>
<summary>Click for options</summary>

1. A) Provides monitoring for AWS resources
2. B) Records AWS API calls
3. C) Automates infrastructure deployment
4. D) Manages user permissions

</details>

<details>
<summary>Click for the answer</summary>
B) Records AWS API calls
</details>

---

## Question 29
Which service is used for building and training machine learning models?

<details>
<summary>Click for options</summary>

1. A) Amazon SageMaker
2. B) Amazon EC2
3. C) AWS Lambda
4. D) Amazon RDS

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon SageMaker
</details>

---

## Question 30
What is the primary purpose of Amazon Elastic Load Balancing?

<details>
<summary>Click for options</summary>

1. A) Distribute incoming application traffic
2. B) Host virtual servers
3. C) Monitor application performance
4. D) Manage user permissions

</details>

<details>
<summary>Click for the answer</summary>
A) Distribute incoming application traffic
</details>

---

## Question 31
Which AWS service provides a fully managed NoSQL database?

<details>
<summary>Click for options</summary>

1. A) Amazon RDS
2. B) Amazon DynamoDB
3. C) Amazon S3
4. D) AWS Lambda

</details>

<details>
<summary>Click for the answer</summary>
B) Amazon DynamoDB
</details>

---

## Question 32
What is Amazon Route 53 used for?

<details>
<summary>Click for options</summary>

1. A) Domain name registration
2. B) Load balancing
3. C) Data storage
4. D) Email sending

</details>

<details>
<summary>Click for the answer</summary>
A) Domain name registration
</details>

---

## Question 33
Which AWS service can be used to create and manage RESTful APIs?

<details>
<summary>Click for options</summary>

1. A) Amazon API Gateway
2. B) AWS Lambda
3. C) Amazon EC2
4. D) Amazon RDS

</details>

<details>
<summary>Click for the answer</summary>
A) Amazon API Gateway
</details>

---

## Question 34
What is the primary use of Amazon Elastic File System (EFS)?

<details>
<summary>Click for options</summary>

1. A) Object storage
2. B) File storage
3. C) Block storage
4. D) Database management

</details>

<details>
<summary>Click for the answer</summary>
B) File storage
</details>

---

## Question 35
Which service helps to track and analyze AWS costs?

<details>
<summary>Click for options</summary>

1. A) AWS Budgets
2. B) Amazon CloudWatch
3. C) AWS Config
4. D) AWS CloudTrail

</details>

<details>
<summary>Click for the answer</summary>
A) AWS Budgets
</details>

---

## Question 36
What does Amazon S3 Stand for?

<details>
<summary>Click for options</summary>

1. A) Simple Storage Service
2. B) Secure Storage System
3. C) Scalable Storage Solution
4. D) Simple Security Service

</details>

<details>
<summary>Click for the answer</summary>
A) Simple Storage Service
</details>

---

## Question 37
Which AWS service provides a centralized view of billing and usage across multiple accounts?

<details>
<summary>Click for options</summary>

1. A) AWS Organizations
2. B) Amazon CloudWatch
3. C) AWS Cost Explorer
4. D) Amazon RDS

</details>

<details>
<summary>Click for the answer</summary>
C) AWS Cost Explorer
</details>

---

## Question 38
Which AWS service provides a scalable file storage solution for use with AWS Cloud services and on-premises resources?

<details>
<summary>Click for options</summary>

1. A) Amazon EBS
2. B) Amazon S3
3. C) Amazon EFS
4. D) Amazon RDS

</details>

<details>
<summary>Click for the answer</summary>
C) Amazon EFS
</details>

---

## Question 39
What is Amazon GuardDuty used for?

<details>
<summary>Click for options</summary>

1. A) Network monitoring
2. B) Threat detection
3. C) Data storage
4. D) Load balancing

</details>

<details>
<summary>Click for the answer</summary>
B) Threat detection
</details>

---

## Question 40
Which AWS service is used to deploy applications in a serverless environment?

<details>
<summary>Click for options</summary>

1. A) Amazon EC2
2. B) AWS Lambda
3. C) Amazon RDS
4. D) Amazon S3

</details>

<details>
<summary>Click for the answer</summary>
B) AWS Lambda
</details>

---

## Question 41
What is the purpose of AWS IAM?

<details>
<summary>Click for options</summary>

1. A) Manage user access and permissions
2. B) Monitor application performance
3. C) Store data
4. D) Manage virtual machines

</details>

<details>
<summary>Click for the answer</summary>
A) Manage user access and permissions
</details>

---

## Question 42
Which service provides a managed Kubernetes environment?

<details>
<summary>Click for options</summary>

1. A) Amazon ECS
2. B) AWS Lambda
3. C) Amazon EKS
4. D) Amazon RDS

</details>

<details>
<summary>Click for the answer</summary>
C) Amazon EKS
</details>

---

## Question 43
What is the primary benefit of using Amazon CloudFront?

<details>
<summary>Click for options</summary>

1. A) Secure data storage
2. B) Global content delivery
3. C) Data processing
4. D) Load balancing

</details>

<details>
<summary>Click for the answer</summary>
B) Global content delivery
</details>

---

## Question 44
Which AWS service is designed for application monitoring and observability?

<details>
<summary>Click for options</summary>

1. A) AWS CloudTrail
2. B) Amazon CloudWatch
3. C) AWS Lambda
4. D) Amazon RDS

</details>

<details>
<summary>Click for the answer</summary>
B) Amazon CloudWatch
</details>

---

## Question 45
Which AWS service can help with resource tagging and cost allocation?

<details>
<summary>Click for options</summary>

1. A) AWS Budgets
2. B) Amazon CloudWatch
3. C) AWS Cost Explorer
4. D) AWS Resource Groups

</details>

<details>
<summary>Click for the answer</summary>
D) AWS Resource Groups
</details>

---

## Question 46
What is the purpose of Amazon Kinesis?

<details>
<summary>Click for options</summary>

1. A) Data streaming service
2. B) Database management
3. C) Virtual machine hosting
4. D) Load balancing

</details>

<details>
<summary>Click for the answer</summary>
A) Data streaming service
</details>

---

## Question 47
Which AWS service is used for serverless event-driven computing?

<details>
<summary>Click for options</summary>

1. A) AWS Lambda
2. B) Amazon EC2
3. C) Amazon RDS
4. D) Amazon S3

</details>

<details>
<summary>Click for the answer</summary>
A) AWS Lambda
</details>

